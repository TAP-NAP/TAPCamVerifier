...
point cloud for relative 3D inspection, with point filtering controls for depth
quality and RGB/depth alignment risks. This research branch also exposes Mesh
RGB as a second render mode in the same filter panel. Mesh RGB reuses the
filtered point selection and hides long stretched depth-jump faces by default;
the `Stretched faces` control can show them for debugging. These visualizations
are not signature inputs and do not change local `valid` / `invalid` semantics.
...
- `src/geometry/` owns signed depth pixel back-projection, decoded RGB analysis
  input, point filtering, and the Three.js point-cloud / Mesh RGB viewer.
...
`test/tap-depth-photo.HEIC` and `test/tap-depth-photo.JPG` are the current local
real-device fixtures generated by the TAPCamDemo content-binding v2 flow. The
repository ignores photo files under `test/` by default so real captures are not
