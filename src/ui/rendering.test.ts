...
describe("renderPixelProjectionPanel", () => {
  ...
  it("renders available relative point cloud metadata and viewer target", () => {
    ...
    expect(html).toContain('class="geometry-mesh-options" data-geometry-mesh-controls hidden');
    expect(html).toContain("Stretched faces");
    expect(html).toContain("Hidden by default");
    expect(html).toMatch(/data-geometry-mesh-stretch[\s\S]*aria-pressed="false"[\s\S]*>\s*Hide\s*<\/button>/);
    expect(html).toContain("Stretch strength");
    expect(html).toContain('data-geometry-stretch-strength type="range" min="0" max="2" step="1" value="1"');
    expect(html).toContain('data-geometry-stretch-strength-label>Medium</b>');
    ...
    expect(html).toContain('<span data-geometry-hidden-stretch>120</span>');
